#version 120
#pragma import_defines(FORCE_OPAQUE, DISTORTION, FORCE_PPL, CLASSIC_FALLOFF, MAX_LIGHTS, ENCODE_NORMALS, TRANSPARENT, NORMALS_ONLY, SHADER_BLENDING, SIMPLE_WATER, FIRST_PERSON, PARTICLE)

#if /*defined(TRANSPARENT) && TRANSPARENT*/ @blend && defined(SHADER_BLENDING) && SHADER_BLENDING
...
#if defined(NORMALS_ONLY) && NORMALS_ONLY && defined(FIRST_PERSON) && FIRST_PERSON
...
vec4 normals = vec4(viewNormal * 0.5 + 0.5, gl_FragData[0].a);
    ...
    gl_FragData[0] = encode(blend(gl_FragData[0], false), blend(normals, true));
#else
