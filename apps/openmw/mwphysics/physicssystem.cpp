...
namespace MWPhysics
{
    ...
    PhysicsSystem::~PhysicsSystem()
    {
        ...
        {
            mTaskScheduler->removeCollisionObject(object.second->getCollisionObject());
    ...
    bool PhysicsSystem::getLineOfSight(const MWWorld::ConstPtr &actor1, const MWWorld::ConstPtr &actor2)
    {
        const auto getWeakPtr = [&](const MWWorld::ConstPtr &ptr) -> const std::weak_ptr<Actor>
        {
    ...
    void PhysicsSystem::remove(const MWWorld::Ptr &ptr)
    {
        ...
        if (found != mObjects.end())
        {
            mTaskScheduler->removeCollisionObject(found->second->getCollisionObject());
    ...
    void PhysicsSystem::updateScale(const MWWorld::Ptr &ptr)
    {
        ...
        if (found != mObjects.end())
        {
            ...
            mTaskScheduler->updateSingleAabb(found->second->getCollisionObject());
            return;
        }
        ActorMap::iterator foundActor = mActors.find(ptr);
        if (foundActor != mActors.end())
        {
            ...
            mTaskScheduler->updateSingleAabb(foundActor->second->getCollisionObject());
            return;
        }
    }
    ...
    void PhysicsSystem::addActor (const MWWorld::Ptr& ptr, const std::string& mesh)
    {
        ...
        Actor* actor = new Actor(ptr, shape, mTaskScheduler.get());
    ...
    std::vector<ActorFrameData> PhysicsSystem::prepareFrameData()
    {
        ...
        for (const auto& m : mMovementQueue)
        {
            ...
            mStandingCollisions[character] = nullptr;
            ...
            actorsFrameData.emplace_back(std::move(physicActor), character, movement, slowFall, waterlevel);
        }
        mMovementQueue.clear();
    ...
    void PhysicsSystem::stepSimulation(float dt)
    {
        for (Object* animatedObject :  mAnimatedObjects)
            animatedObject->animateCollisionShapes(mTaskScheduler.get());
    ...
    void PhysicsSystem::updateAnimatedCollisionShape(const MWWorld::Ptr& object)
    {
        ...
        if (found != mObjects.end())
            found->second->animateCollisionShapes(mTaskScheduler.get());
    }
    ...
    bool LOSRequest::operator==(const LOSRequest& rhs) const
    {
        return mRawActors == rhs.mRawActors;
    }
}
